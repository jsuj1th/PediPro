...
staffAssignmentsRouter.post('/', async (req, res) => {
  ...
  if (!parsed.success) {
    fail(res, 'VALIDATION_ERROR', 'Invalid request', 422, parsed.error.flatten());
    return;
  }

  const { patient_id, template_id, expires_in_days } = parsed.data;
  const auth = req.user as { id: string; practiceId: string };

  // Verify patient belongs to this practice
  const patient = db
    .prepare('select id, child_first_name, child_last_name, child_dob from patients where id = ? and practice_id = ?')
    .get(patient_id, auth.practiceId) as
    | { id: string; child_first_name: string; child_last_name: string; child_dob: string }
    | undefined;
  ...
  const template = db
    ...
    .get(template_id, auth.practiceId) as { id: string; name: string; template_key: string } | undefined;
  ...
  ok(res, {
    ...assignment,
    patient_name: `${patient.child_first_name} ${patient.child_last_name}`,
